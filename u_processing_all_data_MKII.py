...
1. Extract out all storm times, then add lead + recovery times
2. Extract out OMNI + ACE + Supermag for specific storm times (based on stormlist.csv) < -- This will form our individual sets of data 

(for now, we shall continue to feed in data even if Nan values are present)
(for now, we are limited to dmin INTERMAG data)

3. Convert it into a data format that can be taken by our CNN model 
4. Separate out train/val/test data

4. Prep label data 
- Calculate the 99% dBHt value for each storm 
- Create an array for each storm (at 1 minute resolutions), 0 for when dBHt does not exceed the threshold, 1 for when dBHt exceeds the threshold
- Our model should predict 100% when it exceeds threshold and 0% when it does not (ideally)

5. We still need to prep our test data 
...
def get_label(allData_sets):
    ...
    for set in allData_sets: # set is all data for 1 storm
        ...
        tempdf = pd.DataFrame(np.zeros((len(set),2)), columns = ['Exceeded', 'Non Exceeded'])
        ...
        threshold = set['dBHt'].quantile(0.99) # find the 99% dBHt value of each storm

        exceed = set.loc[set['dBHt'] > threshold] # filters out the timings where dBHt > 99% threshold

        for idx in exceed.index.values: 
            tempdf.loc[idx, 'Exceeded'] = 1 # this is our label dataFrame. Storm times where dBHt > 99% threshold is given the label of 1 in the "Exceeded" column

        non_exceed = set.loc[set['dBHt'] < threshold]

        for idx in non_exceed.index.values: 
            tempdf.loc[idx, 'Non Exceeded'] = 1 # this is our label dataFrame. Storm times where dBHt < 99% threshold is given the label of 1 in the "Non Exceeded" column

        # The above works correctly CAA 240425
...
def split_saver(AllFiles, configData): 
    ...
    val = []
    train = []
    val_dir = 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_I/val/'
    train_dir = 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_I/train/'
