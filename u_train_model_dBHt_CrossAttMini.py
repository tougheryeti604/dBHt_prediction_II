'''
Loss is back propagated after every mini batch, rather than each whole batch 
Optimiser.step and Optimiser.zero_grad() is still carried out after every batch
This saves memory 

MINI MINI BATCH BACK PROPAGATION
'''
...
class Transformer(nn.Module): # PUTS OUR ENTIRE TRANSFORMER MODEL TOGETHER --> EMBEDDING + ENCODERS + DECODERS + OUTPUT LAYERS + ACTIVATION FUNCTIONS 
    ...
    def forward(self, target, source = None, mask = None): 
        ...
        source = self.encoder_self_attention.forward(source, None, mask)
        source = self.encoder_self_attention.forward(source, None, mask)
        source = self.encoder_self_attention.forward(source, None, mask) # This final source output from our final Encoder Block is fed into each Decoder block
        ...
        target = self.Output(target) # Linear Output Layer 
...
def optimise(train_Dataset, train_dirs, model_params = None):   
    ...
    num_epoch = 25 # It is the number of times the model trains over the whole dataset
    batch_size = 50
    dataset_size = 50000
    ...
    if model_params != None: 
        print("Loading from checkpoint ... {0} epochs left".format(num_epoch))
    ...
    for epoch in range(num_epoch): # Train our model for the given of epochs 
        ...
        for batch in dataloader: # For every batch of data in our DataLoader 
            ...
            for file in batch: # For each dataset (This can be seen as a mini batch)
                ...
                loss.backward() # Back propagation per mini mini batch
            ...
            optimiser.step() # Update weights and biases // per mini batch
        ...
        torch.save(train_model.state_dict(), 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/checkpoint')
...
def train_loop(train_Dataset, train_dirs): # KICK STARTS TRAINING L
    checkpoint = "C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/checkpoint"
    ...
    torch.save(model_params, 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/model_v1')

    # Save loss values for plotting later
    loss_file = 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/loss.csv'
...
def test(test_Dataset, test_dirs):
    ...
    test_model_params = torch.load('C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/model_v1', weights_only=False)
...
def testnsave(allTest_Dataset, all_test):
    ...
    forecast_file = 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/testforecast.pt'
    ground_truth_file = 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/groundtruth.pt'
    allDates_file = 'C:/Users/UserAdmin/Documents/PythonWork/dbdt_MKII_u/Input_Label_dBHt/assemble_Transformer/config_2/testdates.csv'
